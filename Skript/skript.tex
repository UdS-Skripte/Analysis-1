...
\begin{document}
    ...
    \end{cases}$
     $\forall n \in \NN$ gilt.
   	...
   	\section{Satz}
   	Wenn $\sum_{n =1}^\infty a_n$ konvergiert, so ist $\lim_{n \to \infty} a_n = 0$. Die Umkehrung gilt im Allgemeinen nicht.

   	\section{Bemerkung}
   	Wie $\sum\limits_{n = 1}^\infty \frac{1}{n} =\infty $ zeigt, gilt die Umkehrung jedoch nicht. $\lim\limits_{n = 1}^\infty a_n = 0$ ist zwar notwendig, aber nicht hinreichend für die Konvergenz der Reihe.

   	\lecturedate{23.11.2015}

   	\section{Satz}
   	Eine Reihe $\sum_{n =1}^\infty a_n$ mit $(a_n)_{n \in \NN} \geq 0$ konvergiert genau dann, wenn die Folge $(s_n)_{n \in \NN}$ der Partialsummen beschränkt ist.
   	...
   	\begin{enumerate}
   		...
   		\item
   		$\sum\limits_{n=1}^\infty \frac{1}{n^2}$ konvergiert (gegen $\frac{\pi^2}{6}$)
   	\end{enumerate}

   	\section{Satz: Majorantenkriterium}
   	Ist $\vert a_n\vert \leq \vert c_n \vert, \forall n$ und konvergiert $\sum\limits_{n =1}^\infty \vert c_n \vert$, so konvergiert auch $\sum\limits_{n =1}^\infty \vert a_n \vert$ und es gilt $\left\vert\sum\limits_{n =1}^\infty a_n \right\vert \leq \sum\limits_{n =1}^\infty \vert c_n \vert$

   	\section{Bemerkung}
   	Aus der inversen Dreiecksungleichung folgt, dass
   	\begin{align*}
   		\lim\limits_{n \to \infty} a_n = a \Rightarrow \vert \lim\limits_{n \to \infty} a_n \vert = \vert a \vert
   	\end{align*}
   	...
   	\section{Satz: Leibnitzkriterium}
   	Sei $(a_n)_{n \in \NN}$ eine positive, monoton fallende Nullfolge, dann konvergiert $\sum\limits_{n=1}^\infty {-1}^n a_n$.

   	\lecturedate{25.11.2015}

   	\section{Beispiel}
   	Die alternierende harmonische Reihe konvergiert; jedoch nicht absolut.

   	\section{Definition}
   	Eine Reihe, die konvergiert, jedoch nicht absolut konvergiert, nennen wir bedingt konvergent.


   	\chapter{Umordnung von Reihen}
   	\section{Motivation}
   	Für endliche Summen ist die Reihenfolge der Summierung bezüglich der Summe egal. Bei unendlichen Summen, also Reihen ist dies nicht immer der Fall. 

   	\section{Beispiel}
   	Man kann zum Beispiel die alternierende Harmonische Reihe so umordnen, dass sie scheinbar jeden Grenzwert annimmt.

   	\section{Bemerkung}
   	Aufgrund dieser Eigenschaft bedingt konvergenter Reihen werden wir Summierbarkeit nun weiter untersuchen.

   	\section{Bezeichnung}
   	Sei $I$ eine Indexmenge. Eine Abbildung $(a_i)_{i \in I} \colon I \to \RR$ heißt eine \emph{Familie reeller Zahlen}.
